...
class NavigationEngine {
  ...
  void _onPositionUpdate(Position position) async {
    ...
    if (route.detailedSteps == null || route.detailedSteps!.isEmpty || _currentStepIndex >= route.detailedSteps!.length) return;
  ...
  void _moveToNextStep() {
    if (route.detailedSteps != null && _currentStepIndex < route.detailedSteps!.length - 1) {
      _currentStepIndex++;
      _hasSpokenFar = false;
  ...
  void _updateInstruction({double? distance}) {
    if (route.detailedSteps == null || _currentStepIndex >= route.detailedSteps!.length) return;
...
class _NavigationScreenState extends State<NavigationScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final initialPos = widget.routeOptimization.detailedSteps?.first.address ?? widget.routeOptimization.addresses.first;
