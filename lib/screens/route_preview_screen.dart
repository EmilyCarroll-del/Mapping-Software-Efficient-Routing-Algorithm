...
class _RoutePreviewScreenState extends State<RoutePreviewScreen> {
  ...
  void _setupMapElements() {
    ...
    if (majorStops != null && majorStops.isNotEmpty) {
      for (int i = 0; i < majorStops.length; i++) {
        final step = majorStops[i];
        if (step.address.hasCoordinates) {

          bool isPickup = step.address.fullAddress == widget.order.pickUpAddress.fullAddress;
          bool isDropoff = widget.order.dropOffAddresses.any((a) => a.fullAddress == step.address.fullAddress);
          ...
          _markers.add(
            Marker(
              ...
              icon: BitmapDescriptor.defaultMarkerWithHue(
                ...
                isPickup ? BitmapDescriptor.hueGreen  : // Pickup
                isDropoff ? BitmapDescriptor.hueRed : // Dropoff
                BitmapDescriptor.hueYellow, // Should not happen
              ),
              infoWindow: InfoWindow(
                title: step.address.fullAddress,
                snippet: i == 0 ? 'Start' : (isPickup ? 'Pickup' : 'Drop-off'),
              ),
            ),
          );
        }
  ...
  Widget build(BuildContext context) {
    final displaySteps = widget.routeOptimization.legs != null && widget.routeOptimization.legs!.length > 1
