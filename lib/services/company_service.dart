...
class CompanyService {
  ...
  Future<Company?> getCompanyByCode(String code) async {
    ...
    try {
      final doc = await _db.collection(_companiesCollectionPath).doc(companyId).get();
      if (doc.exists) {
  ...
  /// 
  /// Admins have OPTIONAL companyCode (NOT required).
  /// Returns error message if invalid, null if valid.
  String? validateAdminCompanyCode(String? companyCode) {
    // Admins can have empty/null companyCode (freelancer admins)
    if (companyCode == null || companyCode.trim().isEmpty) {
      return null; // Valid - freelancer admin
    }
